...
- **OAuth tokens never leave your machine.** Each user creates their own Google Cloud OAuth client; this project ships no shared client secret and has no backend. Tokens are stored locally under `~/.config/googledocs-mcp/tokens/` with `0600` permissions.
