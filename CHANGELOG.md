...
> **Breaking: five Drive tools became one.** `list_folder`, `search_drive`, `create_folder`, `copy_doc` and `update_doc` are gone, replaced by `drive({ cmd, args })` speaking `ls` / `find` / `mkdir` / `cp` / `mv`. Anything calling them by name must be updated.

- **Drive navigation is now a filesystem (#44).** Five bespoke tool names became one tool speaking vocabulary the model was trained on, taking the surface from 36 tools to 32. The saving is not only the four slots: `ls` versus `find` needs no explanation where `list_folder` versus `search_drive` did, so the selection problem shrinks from "one of 36" to "one of 32, then one of five inside a namespace it knows cold". Arguments are positional and differ per command on purpose — a shell is not uniform (`ls -la /foo`, `find . -name x`, `mkdir -p /a/b`), and that variability *is* the pre-trained pattern; making them uniform would invent a shape that has to be learned, which is the thing this avoids. Paths are `/` or `~` for My Drive, `/shared/<drive name>` for a shared drive, `/shared-with-me`, and `/lost+found`; anything not starting with `/` or `~` is read as a Drive id or URL, so ids from any other tool paste straight in. A whole path resolves in a single `files.list` call — every segment name is fetched at once and the parent graph is walked locally — which in the common case is *fewer* round trips than before, since finding a doc by name already cost a search first.
- **`ls /lost+found` replaces `list_folder({ folder: "orphaned" })`.** Same scan, Unix's name for exactly this.
- **The Drive vocabulary is borrowed only where it is honest.** Three places it is not, each of which refuses rather than half-working: **(1)** Drive permits two files with one name in one folder and folds case when matching — no filesystem the model learned from does either, so it would never think to check. A path matching more than one thing is refused with the candidates listed, and `cp`/`mv` refuse to *create* that state too rather than manufacturing an ambiguity the resolver would then decline to resolve. **(2)** `cp -r` does not exist: verified live, Drive's `files.copy` answers "This file cannot be copied by the user" for a folder, and its own web UI cannot copy one either. **(3)** `mv` into `/shared/…` transfers ownership to that organization and cannot be undone from your side — shell `mv` across filesystems leaves you owning the file, so this requires `acceptOwnershipTransfer: true`. `mv` also keeps `update_doc`'s guard as `expectName`.
- **Flags and operands parse in any order**, as a terminal accepts them: `cp -r a b`, `cp a b -r` and `cp a -r b` are one command. `--` ends the options, which is the shell's own escape hatch and pre-trained like the rest. What an unrecognised `-token` means is set per command rather than guessed once, because the commands disagree: `ls -la /Work` wants the flag ignored and the path used, while `find -2026` wants `-2026` searched for.
- **`cp` keeps the source name.** Drive's `files.copy` defaults to "Copy of …", which is its UI convention; `cp file /dir` on a filesystem produces `file`. Found on the live run, not in unit tests.
- **No destructive command ships.** There was none in the surface to collapse, and host permissions are granted per tool *name* — a user who allowlists `drive` to stop being prompted for `ls` would be allowlisting `rm` too. The conditions that would change that are recorded in #47.

### Documentation
- **The README shows what to ask for, above the fold.** Two chained examples — filling a template without rebuilding it, and a whole review pass in one instruction — sit right after "Why this exists", where previously the only pointer to `docs/recipes.md` was a blockquote buried under the tool table. `docs/recipes.md` gains a **Whole jobs** section for asks that fan out across several tools, above the existing single-tool recipes. Every claim in the template example was verified live: a `cp` preserves the letterhead, custom margins and landscape orientation, the whole-document font, and the embedded image at its exact size (118.26 x 40pt on both sides).

### Added
- **Files in no folder are listable** (`ls /lost+found`, shipped as `list_folder({ folder: "orphaned" })` and renamed by #44 in the same release) — the files you own that sit in no folder at all. They open, they turn up in a search, and nothing that browses the tree will ever show them, so they are found only by someone who already remembers they exist. You cannot create this state yourself (removing a file's only parent reparents it to My Drive root) — it is the residue of someone else deleting or unsharing a folder your files were in, which is why nobody thinks to look. Drive has no query operator for "has no parent", so the scan pages files and filters here: it asks only for files you own (shared-with-me files are legitimately parentless and not yours to re-home), it is bounded, and a scan that hit the bound says so rather than passing a cap off as the whole answer. Re-home one with `update_doc({ folder })`, which already moves any Drive file. Kept as a value of the existing `folder` parameter rather than a new tool — one field, one type, whatever you are listing, the way Drive's own sidebar makes "Shared with me" a place (#46).
